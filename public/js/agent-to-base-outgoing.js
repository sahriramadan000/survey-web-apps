        ...
        new gridjs.Grid({
            columns: [
                ...
                },
                {
                    ...
                    width: "130px",
                    formatter: (_, row) => {
                        const outgoingId = row.cells[9].data;

                        return gridjs.html(`
                            <button onclick="editAgentToBaseOutgoing('${outgoingId}')" class="btn btn-sm btn-soft-secondary me-1">
                                <i class="bx bx-edit fs-16"></i>
                            </button>
                            <button onclick="deleteAgentToBaseOutgoing('${outgoingId}')" class="btn btn-sm btn-soft-danger">
            ...
            search: true,
            server: {
                url: "/agent-to-base-outgoing-list",
                then: data => {
...
// Fungsi Edit Product
function editAgentToBaseOutgoing(outgoingId) {
    let editUrl = editOutgoingProductUrl.replace(":id", outgoingId);
    window.location.href = editUrl;
}

// Fungsi Delete Product
function deleteAgentToBaseOutgoing(outgoingId) {
    Swal.fire({
